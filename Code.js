function doGet(e) {
  try {
    ...
    if (params.action === 'getRecordSignatures') {
      var sigs = getRecordSignatures(params.recordId || '');
      return ContentService.createTextOutput(JSON.stringify(sigs))
          .setMimeType(ContentService.MimeType.JSON);
    }

    if (params.action === 'getEventSignatures') {
      var evtSigs = getEventSignatures(params.eventId || '');
      return ContentService.createTextOutput(JSON.stringify(evtSigs))
          .setMimeType(ContentService.MimeType.JSON);
    }
...
function getInitialData() {
  ...
  for (var j = 1; j < recordData.length; j++) {
    ...
    records.push({
      ...
      repairResult: s(row[17]), satisfaction: s(row[18]), repairDetails: s(row[19]), returnAction: s(row[20]),
      signature: "", // 效能最佳化：初始同步不傳送龐大 Base64 手寫簽名（原佔 9.5MB），按需讀取極速加速
      hasSignature: !!s(row[21]), // 保留簽名是否存在標記
      ...
      returnSignature: "", // 效能最佳化：初始同步不傳送龐大 Base64 領回簽名（原佔 2.0MB），按需讀取
      hasReturnSignature: !!s(row[36]), // 保留領回簽名是否存在標記
      gender: s(row[37]),
      ipAddress: s(row[38]),
      checkinNumber: cNo,
      checkedIn: isChecked,
...
// 按需讀取單筆紀錄的手寫簽名（送修簽名與領回簽名）
function getRecordSignatures(recordId) {
  if (!recordId) return { signature: "", returnSignature: "" };
  var sheets = setupSheets();
  var recordData = sheets.recordSheet.getDataRange().getValues();
  for (var j = 1; j < recordData.length; j++) {
    var recId = s(recordData[j][33]) || ("REC_OLD_" + j);
    if (recId === recordId) {
      return {
        signature: s(recordData[j][21]),
        returnSignature: s(recordData[j][36])
      };
    }
  }
  return { signature: "", returnSignature: "" };
}

// 列印簽到退名冊時才按需讀取整場次簽名
function getEventSignatures(eventId) {
  if (!eventId) return {};
  var sheets = setupSheets();
  var recordData = sheets.recordSheet.getDataRange().getValues();
  var result = {};
  for (var j = 1; j < recordData.length; j++) {
    var recEvtId = s(recordData[j][34]) || (s(recordData[j][1]) === "區隊通報" ? "MODE_C_QUEUE" : "");
    if (recEvtId === eventId) {
      var recId = s(recordData[j][33]) || ("REC_OLD_" + j);
      result[recId] = {
        signature: s(recordData[j][21]),
...
function saveToSheet(formData) {
  ...
  try {
    ...
    // 支援品牌與物品名稱分開傳入之處理
    if (formData.brandName && formData.itemName) {
      formData.brandModel = s(formData.brandName) + ' ' + s(formData.itemName);
    } else if (formData.itemName && !formData.brandModel) {
      formData.brandModel = s(formData.itemName);
    }
    // 物品名稱必填且不得含數字驗證（僅對一般線上登記檢查）
    if (formData.itemName && /[0-9]/.test(formData.itemName)) {
      throw new Error('ITEM_NAME_INVALID:物品名稱只能填寫中英文，不能填寫數字。');
    }

    // 全面禁止品牌防護（小米／米家／Xiaomi／Dyson）
    if (isUnsupportedBrand_(formData.brandModel, formData.brandName, formData.itemName, formData.category, formData.problem, formData.accessories)) {
    ...
    if (s(formData.photoData).length > 49000) throw new Error('PHOTO_TOO_LARGE:照片壓縮後仍過大，請重新選擇照片。');
    if (s(formData.signature).length > 49000 || s(formData.returnSignature).length > 49000) {
